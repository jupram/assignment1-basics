...
def main():
    ...
    #serialize the vocab and merges to json files respectively
    import json
    with open("vocab_s.json", "w", encoding="utf-8") as f:
        json.dump({str(k): v.decode("utf-8", errors="ignore") for k, v in v.items()}, f, ensure_ascii=False, indent=4)
    with open("merges_s.json", "w", encoding="utf-8") as f:
        json.dump([[a.decode("utf-8", errors="ignore"), b.decode("utf-8", errors="ignore")] for a, b in m], f, ensure_ascii=False, indent=4)
